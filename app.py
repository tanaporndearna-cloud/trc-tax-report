...
PRODUCT_KEYWORDS = {
    "ชุดกันชนหน้า","ชุดกันชนหลัง","ชุดไฟท้าย","ชุดไฟหน้า","ชุดไฟ",
    "ชุดกันชนรอบคัน","ชุดแต่งรอบคัน","ชุดซันรูฟ","ชุดกันชนหน้า-หลัง",
    "ชุดแปลงกันชนหน้า-หลัง","ชุดสเกิร์ตรอบคัน","สเกิร์ตรอบคัน",
    "สปอร์ตไลท์","ไฟ","ไฟท้าย","ไฟหน้า","ไฟเลี้ยว","ไฟตัดหมอก",
    "ไฟทับทิมท้าย","ไฟติดขอบบันได","ไฟสปอยเลอร์","LED","DRL",
    "กันชนหน้า+หลัง","กันชนหน้า","กันชนหลัง","กันชนข้าง",
    "ลิ้นหน้า","ลิ้นหลัง","ลิ้นกันชน","ลิ้นต่อกันชนหน้า",
    "สเกิตร์ข้าง","สเกิร์ตข้าง","หน้ากระจัง","กระจังหน้า",
    "ฝาหน้า","ฝาครอบสปอร์ตไลท์","บันไดข้าง",
    "สปอยเลอร์","สปอยเลอร์หลัง","สปอยเลอร์หน้า","สปอยเลอร์เล็ก","สปอยเลอร์ใหญ่",
    "เสื้อไฟท้าย (ซ้าย)","เสื้อไฟท้าย (ขวา)","เสื้อไฟท้าย",
    "ตะแกรง","กรองอากาศ","ฝากระโปรง","กรอบป้ายทะเบียนพลาสติก",
    "หลังคา","ขอบประตู","แผ่นตะแกรงรถ หน้า-หลัง มีไฟ","แผ่นกันรอยใต้ท้องรถ",
    "กระจกมองข้าง","กระจกมองหลัง","เดย์ไลท์","จานเบรค","กาบข้าง",
    "กล่องใส่ตะแกรงอลูมิเนียม","ใบปัดน้ำฝน","สติ๊กเกอร์","หน้าปัดไมล์","กล้อง CAMERA",
}
...
def abbreviate_item(text):
    ...
    if not ym:
        parts_t = t.split()
        first_t = parts_t[0] if parts_t else ""
        if re.match(r'^[A-Za-z0-9\-]+$', first_t):
            thai_t = next((p for p in parts_t[1:] if re.search(r'[฀-๿]', p)), None)
            check_word = thai_t if thai_t else first_t
        else:
            check_word = first_t
        if not any(kw in check_word for kw in PRODUCT_KEYWORDS):
    ...
    if product:
        ...
        if re.match(r'^[A-Za-z0-9\-]+$', first):
            ...
            product = first
        if not any(kw in product for kw in PRODUCT_KEYWORDS):
            product = "อุปกรณ์รถยนต์"
